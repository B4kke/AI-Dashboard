...
let state = { projects: [], ideas: [], tasks: [], agents: [], runs: [], researchRuns: [], modelProviders: [], integrations: {} };
let openCodeModels = [];
...
function renderRuns() {
  $('run-list').innerHTML = state.runs.length ? state.runs.slice().reverse().map((run) => {
    ...
    return `<div class="row-card"><div><div class="title">${escapeHtml(run.kind || 'worker')} · ${escapeHtml(run.runner)}</div><div class="meta">${escapeHtml(modelLabel(run.model))} · ${escapeHtml(run.branch || run.sessionId || run.taskId || 'run')}${evidence}${run.error ? `<br>${escapeHtml(run.error)}` : ''}</div></div><div class="row-actions"><span class="tag">${escapeHtml(run.status)}</span>${['running','retrying'].includes(run.status) ? `<button class="abort" data-run="${run.id}">Abort</button>` : ''}</div></div>`;
...
function renderAutonomy() {
  $('autonomy-list').innerHTML = state.projects.length ? state.projects.map((project) => {
    ...
    return `<div class="row-card"><div><div class="title">${escapeHtml(project.name)}</div><div class="meta">${escapeHtml(config.supervisorRole || 'supervisor')} supervisor · max ${config.maxTaskIterations || 4} iterations · ${config.autoMerge ? 'auto-merge on' : 'manual merge'}<br>${escapeHtml(gate)}</div></div><div class="row-actions"><span class="tag">${escapeHtml(config.mode || 'manual')}</span>${config.mode !== 'manual' ? `<button class="tick" data-project="${project.id}">Run loop</button>` : ''}</div></div>`;
...
function render() {
  ...
  renderProjects(); renderIdeas(); renderTasks(); renderRuns(); renderResearch(); renderProviders(); renderAutonomy(); renderProjectOptions(); renderModelLists();
}

async function refresh() {
  try {
    const [nextState, health, providers] = await Promise.all([api('/api/state'), api('/api/health'), api('/api/model-providers').catch(() => [])]);
    state = nextState; if (Array.isArray(providers)) state.modelProviders = providers;
...
$('project-form').addEventListener('submit', async (event) => {
  ...
  const data = {
    ...
    autonomy: { mode: raw.autonomyMode || 'manual', requireCi: raw.requireCi === 'on', autoAnalyzeIdeas: raw.autoAnalyzeIdeas === 'on', autoMerge: raw.autoMerge === 'on' },
  };
  await api('/api/projects', { method: 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify(data) }); $('project-dialog').close(); form.reset(); await refresh();
});
...
for (const type of ['project.created','project.updated','idea.created','idea.updated','task.created','task.updated','run.created','run.updated','research.created','research.updated','model-provider.created','model-provider.updated','integration.updated']) {
