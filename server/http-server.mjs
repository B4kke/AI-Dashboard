...
export function createHttpServer({ store, events, orchestrator, autonomy, research, github, publicDir, version = '0.0.5' }) {
  async function api(request, response, url) {
    if (request.method === 'GET' && url.pathname === '/api/health') {
      const [openCode, providers] = await Promise.all([
        orchestrator.opencodeOverview(),
        research.listProviders().catch(() => []),
      ]);
      return json(response, 200, {
        ...
        persistence: store.persistenceInfo?.() || { type: 'unknown' },
    ...
    if (request.method === 'GET' && url.pathname === '/api/integrations/opencode') {
      const value = await orchestrator.opencodeOverview();
    ...
    if (request.method === 'GET' && url.pathname === '/api/integrations/opencode/models') {
      const value = await research.openCodeModels(url.searchParams.get('projectId'));
      return json(response, 200, value);
    }
    if (request.method === 'GET' && url.pathname === '/api/integrations/github') {
      const value = await orchestrator.githubOverview(url.searchParams.get('repository'));
    ...
    if (request.method === 'GET' && taskEvidence) {
      const taskId = decodeURIComponent(taskEvidence[1]);
  ...
  return createServer(async (request, response) => {
    try {
      ...
      const status = /not found/i.test(error.message) ? 404 : /already in progress|cannot .* from state/i.test(error.message) ? 409 : 500;
