...
import { taskWorkScopes } from './work-scope.mjs';

function jsonContract(shape) {
  return ['', 'Your final response MUST end with this exact marker followed by one JSON code block:', RESULT_MARKER, '```json', JSON.stringify(shape, null, 2), '```', 'Do not put any text after that JSON block.'];
}
...
export function buildTaskPrompt({ project, task, feedback = null, iteration = 1 }) {
  const lines = [`Work on the delegated task: ${task.title}`, '', `Project: ${project.name}`, `Priority: ${task.priority}`, `Iteration: ${iteration}`, ...projectBrief(project), ...specialistContext(task)];
  ...
  lines.push('', 'Execution contract:',
    ...
    '- Do not modify unrelated files or another specialist’s work scope.',
    ...
    ...jsonContract({ schemaVersion: RESULT_SCHEMA_VERSION, kind: 'worker', status: 'success', summary: 'What changed and why', evidence: { tests: ['tests/checks you actually ran and their observed outcome'], notes: ['other concrete evidence or constraints'] }, risks: [], needsInput: null }));
  return lines.join('\n');
}

export function buildPlannerPrompt({ project, idea }) {
  return [`Turn this product idea into an executable implementation plan for ${project.name}.`, ...projectBrief(project), '', `Idea: ${idea.title}`, idea.description || '', '', 'Planning contract:',
