...
export class StateStore {
  ...
  async updateIdea(id, patch) {
    ...
    Object.assign(idea, patch, { updatedAt: new Date().toISOString() }); await this.#changed('idea.updated', idea); return structuredClone(idea);
  }
  ...
  async updateRun(id, patch) {
    ...
    Object.assign(run, patch, { updatedAt: new Date().toISOString() }); await this.#changed('run.updated', run); return structuredClone(run);
  }

  async upsertModelProvider(input) {
    const existing = this.state.modelProviders.find((item) => item.id === input.id);
    const now = new Date().toISOString();
    const value = {
      ...(existing || {}), ...input,
    ...
    await this.#changed(existing ? 'model-provider.updated' : 'model-provider.created', value);
  ...
  async updateResearchRun(id, patch) {
    const run = this.state.researchRuns.find((item) => item.id === id);
    if (!run) throw new Error('Research run not found');
    Object.assign(run, patch, { updatedAt: new Date().toISOString() }); await this.#changed('research.updated', run); return structuredClone(run);
  }
