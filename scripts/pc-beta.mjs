...
class BetaHarness {
  ...
  async prepareRepository() {
    ...
    const remote = await git(this.config.repoPath, ['remote', 'get-url', 'origin']);
    if (parseGitHubRemote(remote) !== this.config.repository) throw new Error(`origin ${remote} does not match ${this.config.repository}`);
    ...
    this.session.evidence.fixture = { baseBranch, head, paths, remote };
    await this.persist();
    return this.session.evidence.fixture;
  }

  projectInput() {
    const codingModel = clean(process.env.AI_DASHBOARD_BETA_CODING_MODEL) || null;
  ...
  async ensureProject() {
    if (this.session.projectId) {
      const state = await this.state();
      if (state.projects.some((project) => project.id === this.session.projectId)) return state.projects.find((project) => project.id === this.session.projectId);
    }
  ...
  async run() {
    ...
    this.session.dashboardCommit = await git(ROOT, ['rev-parse', 'HEAD']);
    await this.persist();
    ...
    await this.scenario('happy_path', async () => {
      const evidence = await this.runAutonomousTask(specs.happy);
      this.session.evidence.happyPath = { taskId: evidence.task.id, prNumber: evidence.task.publication.prNumber, checkpoint: evidence.workers.at(-1).checkpointHead, workerTreeSha: evidence.task.publication.workerTreeSha, workerBaseSha: evidence.task.publication.workerBaseSha, mergeSha: evidence.task.publication.mergeSha };
    ...
    await this.scenario('ci_failure_repair', async () => {
      const evidence = await this.runAutonomousTask(specs.ciRepair);
    ...
    await this.scenario('supervisor_rejection', async () => {
      const evidence = await this.runAutonomousTask(specs.supervisorReject);
