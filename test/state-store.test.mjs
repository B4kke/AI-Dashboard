...
test('state store persists task model selection and provider/research state', async () => {
  const dir = await mkdtemp(join(tmpdir(), 'ai-dashboard-'));
  try {
    const file = join(dir, 'state.json');
    const store = new StateStore(file); await store.load();
    const project = await store.addProject({ name: 'Test', repoPath: '/tmp/project', modelPolicy: { codingModel: 'lmstudio/qwen3', researchModel: 'nvidia/meta/llama' } });
    const task = await store.addTask({ projectId: project.id, title: 'First task' });
